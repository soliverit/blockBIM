from .props import *
from .dce_model_object_set_group import SbemObjectSetGroup
###### SBEM OBJECT SET ######
# Effectively a general object set (see sbem_object.py)
# Store collection of SbemObjects to offer consistent storage
# and matrix operations
#
# NOTE TO SELF: I bet you can base this on the abstract SbemObject to spare having the
# duplicated magic methods. Maybe not __getattr__() though
#
...
class DCEObjectSet():
    ...
    # Add magic method so we can combine SbemObjectSets
    def __add__(self, other):
        returnSbemObjectSet = self.__class__()
        if self.__class__ == other.__class__:
            for obj in self.objects:
                returnSbemObjectSet.append(obj)
                
            for obj in other.objects:
                if obj not in returnSbemObjectSet:
                    returnSbemObjectSet.append(obj)
    ...
    # Access described in sbem_object_set_group, basically group[<key>]
    def groupBy(self,name):
        outputs = SbemObjectSetGroup(name, self.__class__)
        for obj in self.objects:
